...
sys.path.insert(0, "./")
BlenderExporterDir = os.getenv("BlenderExporter", os.path.dirname(os.path.abspath(__file__)))
...
def menu_export_osg_model(self, context):
    ...
    # self.layout.operator(ExportOperator.bl_idname, text="OSG Model(.osg)").filepath = default_path
    self.layout.operator(ExportOperator.bl_idname, text="OSG Model(.osgt)")
...
class ExportOperator(bpy.types.Operator, ExportHelper):
    ...
    def draw(self, context):
        ...
        layout.row(align=True).label(text="Author:")
        ...
        layout.row(align=True).label(text="Texture Prefix:")
    ...
    def invoke(self, context, event):
        ...
        return super(ExportOperator, self).invoke(context, event)
